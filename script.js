(() => {
  ...
  function handleDataURL(dataURL){
    ...
    img = new Image();
    img.onload = () => { imgLoaded = true; points=[]; aiBtn.disabled = false; draw(); log("画像 onload 完了。"); };
    img.onerror = () => { log("⚠️ img.onerror: 画像の読み込みに失敗しました。"); };
    img.src = dataURL;
  }
  ...
  function handleFile(file){
    ...
    r.onload = () => {
      if (typeof r.result !== "string"){ log("⚠️ FileReader 結果が文字列ではありません。"); return; }
