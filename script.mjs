...
let img = new Image(); let imgLoaded=false; let points=[];
...
function draw(){
  ...
  if(points.filter(Boolean).length>=2){
    ...
    for(let i=0;i<points.length-1;i++){ const a=points[i], b=points[i+1]; if(!a||!b) continue; ctx.moveTo(a.x,a.y); ctx.lineTo(b.x,b.y); } ctx.stroke(); ctx.restore();
  }
  points.forEach((p,i)=>{
    if(!p) return;
    ctx.save(); ctx.beginPath(); ctx.fillStyle=COLORS[i]; ctx.arc(p.x,p.y,RADIUS,0,Math.PI*2); ctx.fill();
    ...
    ctx.textAlign="center"; ctx.textBaseline="middle"; ctx.strokeText(label,p.x,p.y); ctx.fillStyle="#fff"; ctx.fillText(label,p.x,p.y); ctx.restore();
  });
}
