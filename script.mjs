...
function setImageFromURL(url){
  ...
  img.onerror = () => { log("⚠️ img.onerror（ObjectURL）: 読み込み失敗。FileReaderにフォールバックします。"); if(lastObjectURL){ URL.revokeObjectURL(lastObjectURL); lastObjectURL=null; } };
  img.src = url;
}
function setImageFromDataURL(dataURL){
  ...
  img.onerror = () => { log("⚠️ img.onerror（DataURL）: 読み込み失敗。"); };
  img.src = dataURL;
}
function handleFile(file){
  ...
  try{
    ...
    log("ObjectURL 生成成功。"); setImageFromURL(url); return;
  }catch(e){ log("⚠️ ObjectURL 生成失敗: "+e.message); }
  // Fallback to FileReader
  try{
    const reader = new FileReader();
    reader.onload = () => { if(typeof reader.result === 'string'){ log("FileReader 読み込み成功。"); setImageFromDataURL(reader.result); } else { log("⚠️ FileReader 結果が文字列ではありません。"); } };
    reader.onerror = () => { log("⚠️ FileReader 読み込みに失敗しました。"); };
    reader.readAsDataURL(file);
  }catch(e){ log("⚠️ FileReader 例外: "+e.message); }
}
...
// Paste from clipboard (iOS 15+ Safari では許可確認が出ることあり)
window.addEventListener('paste', async (e)=>{
  try{
    const items = e.clipboardData?.items || [];
    for(const it of items){
      if(it.type.startsWith('image/')){ const file = it.getAsFile(); if(file) { log("クリップボードから画像を取得。"); handleFile(file); return; } }
    }
...
// ----- MoveNet -----
let detector=null;
async function ensureDetector(){ if(detector) return detector; await tf.setBackend('webgl'); await tf.ready(); log("TensorFlow.js backend: "+tf.getBackend()); detector = await posedetection.createDetector(posedetection.SupportedModels.MoveNet, { modelType: 'Lightning' }); log("MoveNet detector 初期化完了。"); return detector; }
...
async function runAIDetect(){ try{ if(!imgLoaded){ log("⚠️ 先に画像を読み込んでください。"); return; } const det=await ensureDetector(); const off=document.createElement('canvas'); off.width=canvas.width; off.height=canvas.height; const octx=off.getContext('2d'); octx.drawImage(img,0,0,off.width,off.height); const poses=await det.estimatePoses(off,{flipHorizontal:false}); if(!poses||!poses.length){ log("⚠️ 検出できませんでした。全身が写る横向き写真でお試しください。"); return; } const kp=poses[0].keypoints; let side=sideSelect.value; if(side==='auto') side=sideByScore(kp); const map=Object.fromEntries(kp.map(k=>[k.name,k])); const sel= side==='left' ? [map['left_ear'],map['left_shoulder'],map['left_hip'],map['left_knee'],map['left_ankle']] : [map['right_ear'],map['right_shoulder'],map['right_hip'],map['right_knee'],map['right_ankle']]; points=sel.map(k=>({x:k.x,y:k.y})); plumbX=Math.round(points[4].x+8); plumbXInput.value=plumbX; draw(); compute(); log(`AI検出完了（側: ${side}）。`);} catch(e){ log("AIエラー: "+(e&&e.message?e.message:e)); } }
